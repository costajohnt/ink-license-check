...
export function formatText(results, { version = 'unknown' } = {}) {
  ...
  for (const r of results) {
    if (r.error) {
      ...
      lines.push(`  ${c.yellow}${c.bold}n/a ${c.reset}  ${c.bold}${r.package}@${r.version}${c.reset}${dl}`);
      lines.push(`  ${c.dim}      Uses ink via dependency (not bundled) — attribution obligation falls on install, not this tarball${c.reset}`);
...
export function formatReport(results, { version = 'unknown', minDownloads = null } = {}) {
  ...
  const dependencyOnly = results.filter((r) => r.status === 'na');
  ...
  lines.push(`| Bundles Ink | ${violations.length + passing.length} |`);
  lines.push(`| **Violations** | **${violations.length}** |`);
  lines.push(`| Properly attributed | ${passing.length} |`);
  if (dependencyOnly.length > 0) lines.push(`| Uses Ink via dependency (not bundled, n/a) | ${dependencyOnly.length} |`);
  ...
  // Dependency-only table (informational, not a violation)
  if (dependencyOnly.length > 0) {
    lines.push('## Uses Ink via dependency (not bundled)');
    lines.push('');
    lines.push('These packages declare Ink as a dependency but do not bundle its source, so npm installs Ink separately and they carry no attribution obligation in their own tarball.');
    lines.push('');
    lines.push('| Package | Version | Downloads |');
    lines.push('|---------|---------|-----------|');
    for (const r of dependencyOnly) {
      const dl = r.downloads != null ? formatNumber(r.downloads) : '-';
      lines.push(`| ${r.package} | ${r.version} | ${dl} |`);
    }
    lines.push('');
...
function countStatuses(results) {
  const summary = { total: results.length, pass: 0, fail: 0, na: 0, skip: 0, error: 0 };
